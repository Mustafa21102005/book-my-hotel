        ...
        commands: {
            ...
            today: {
                ...
                enabled: function (inst) {
                    ...
                    return (
                        (!minDate ||
                            plugin.today().getTime() >= minDate.getTime()) &&
                        (!maxDate ||
                            plugin.today().getTime() <= maxDate.getTime())
                    );
                },
                date: function () {
                    return plugin.today();
            ...
            prevWeek: {
                ...
                enabled: function (inst) {
                    ...
                    return (
                        !minDate ||
                        plugin
                            .add(plugin.newDate(inst.drawDate), -7, "d")
                            .getTime() >= minDate.getTime()
                    );
                },
                date: function (inst) {
            ...
            prevDay: {
                ...
                enabled: function (inst) {
                    ...
                    return (
                        !minDate ||
                        plugin
                            .add(plugin.newDate(inst.drawDate), -1, "d")
                            .getTime() >= minDate.getTime()
                    );
                },
                date: function (inst) {
...
monthsOffset: function(date) { // Always start on the quarter
  ...
  alert('Selected ' + dates);
        ...
        formatDate: function (format, date, settings) {
            ...
            var monthNames =
                settings.monthNames || this.defaultOptions.monthNames;
            var calculateWeek =
        ...
        parseDate: function (format, value, settings) {
            ...
            var getNumber = function (match, step) {
                ...
                var size = [2, 3, isDoubled ? 4 : 2, 11, 20][
                    "oy@!".indexOf(match) + 1
                ];
                var digits = new RegExp("^-?\\d{1," + size + "}");
                var num = value.substring(iValue).match(digits);
                if (!num) {
                    throw "Missing number at position {0}".replace(
                        /\{0\}/,
                        iValue,
                    );
                }
            ...
            var checkLiteral = function () {
                if (value.charAt(iValue) !== format.charAt(iFormat)) {
                    throw "Unexpected literal at position {0}".replace(
                        /\{0\}/,
                        iValue,
                    );
                }
...
var doy = $.datepick.dayOfYear(new Date(2014, 12-1, 25)) */
        dayOfYear: function (year, month, day) {
            ...
            return (
                Math.floor(
                    (date.getTime() - newYear.getTime()) / plugin._msPerDay,
                ) + 1
            );
        },
...
var week = $.datepick.iso8601Week(new Date(2014, 12-1, 25)) */
        iso8601Week: function (year, month, day) {
            ...
            return (
                Math.floor(
                    Math.round((time - checkDate) / plugin._msPerDay) / 7,
                ) + 1
            );
        },
...
$.datepick.newDate(2014, 12, 25) */
        ...
        add: function (date, amount, period) {
            if (period === "d" || period === "w") {
                ...
                date.setDate(
                    date.getDate() + amount * (period === "w" ? 7 : 1),
                );
            } else {
        ...
        _instSettings: function (elem) {
            return {
                ...
                pickingRange: false,
                inline:
                    $.inArray(elem[0].nodeName.toLowerCase(), ["div", "span"]) >
                    -1,
                get: function (name) {
                    // Get a setting value, computing if necessary
                    if (
                        $.inArray(name, ["defaultDate", "minDate", "maxDate"]) >
                        -1
                    ) {
        ...
        _postAttach: function (elem, inst) {
            if (inst.inline) {
                ...
                elem.on("keydown." + inst.name, this._keyDown)
        ...
        _optionsChanged: function (elem, inst, options) {
            if (
                options.calendar &&
                options.calendar !== inst.options.calendar
            ) {
        ...
        _attachments: function (elem, inst) {
            ...
            inst.trigger = !trigger
                ...
                : $(trigger)
                      ...
                      [inst.options.isRTL ? "insertBefore" : "insertAfter"](
                          elem,
                      )
                      .click(function () {
                          if (!plugin.isDisabled(elem[0])) {
                              plugin[plugin.curInst === inst ? "hide" : "show"](
                                  elem[0],
                              );
                          }
                      });
            ...
            if (
                ...
                this.setDate(
                    elem[0],
                    plugin.newDate(defaultDate || plugin.today()),
                );
            }
        },
        ...
        _autoSize: function (elem, inst) {
            if (inst.options.autoSize && !inst.inline) {
                ...
                inst.elem.attr(
                    "size",
                    plugin.formatDate(dateFormat, date, inst.getConfig())
                        .length,
                );
            }
        },
        ...
        enable: function (elem) {
            ...
            if (inst.inline) {
                elem.children("." + this._disableClass)
                    .remove()
                    .end()
                    .find("button,select")
                    .prop("disabled", false)
            ...
            if (!$.isEmptyObject(inst)) {
                if (inst.inline || plugin.curInst === inst) {
                    if (
                        ...
                        (!inst.prevDate ||
                            inst.prevDate.getFullYear() !==
                                inst.drawDate.getFullYear() ||
                            inst.prevDate.getMonth() !==
                                inst.drawDate.getMonth())
                    ) {
                ...
                if (inst.inline) {
                    ...
                    focus
                        .eq(Math.max(Math.min(index, focus.length - 1), 0))
                        .focus();
        ...
        _checkOffset: function (inst) {
            var base =
                inst.elem.is(":hidden") && inst.trigger
                    ? inst.trigger
                    : inst.elem;
            ...
            var above =
                offset.top - (isFixed ? scrollY : 0) - inst.div.outerHeight();
            var below =
                offset.top - (isFixed ? scrollY : 0) + base.outerHeight();
        ...
        _checkExternalClick: function (event) {
            ...
            if (
                elem.closest(
                    "." + plugin._popupClass + ",." + plugin._triggerClass,
                ).length === 0 &&
            ...
            if (inst.inline || inst.div) {
                if (event.keyCode === 9) {
                    ...
                    plugin.selectDate(
                        elem,
                        $(
                            "a." + inst.options.renderer.highlightedClass,
                            inst.div,
                        )[0],
                    );
                    handled = true;
        ...
        _doMouseWheel: function (event, delta) {
            ...
            if (inst.options.useMouseWheel) {
                ...
                plugin.changeMonth(
                    elem,
                    -inst.options[
                        event.ctrlKey ? "monthsToJump" : "monthsToStep"
                    ] * delta,
                );
            }
            event.preventDefault();
...
$(selector).datepick('setDate', [date1, date2, date3]) */
                ...
                for (var i = 0; i < dates.length; i++) {
                    ...
                    if (date) {
                        if (
                            ...
                            for (
                                var j = 0;
                                j < inst.selectedDates.length;
                                j++
                            ) {
                                if (
                                    inst.selectedDates[j].getTime() ===
                                    date.getTime()
                                ) {
                                    found = true;
                                    break;
        ...
        retrieveDate: function (elem, target) {
            ...
            return $.isEmptyObject(inst)
                ...
                : this._normaliseDate(
                      new Date(
                          parseInt(
                              target.className.replace(/^.*dp(-?\d+).*$/, "$1"),
                              10,
                          ),
                      ),
                  );
        ...
        selectDate: function (elem, target) {
            ...
            if (!$.isEmptyObject(inst) && !this.isDisabled(elem)) {
                ...
                if (inst.options.multiSelect) {
                    ...
                    for (var i = 0; i < inst.selectedDates.length; i++) {
                        if (
                            date.getTime() === inst.selectedDates[i].getTime()
                        ) {
        ...
        _generateContent: function (elem, inst) {
            ...
            if (monthsToShow[1] > 1) {
                var count = 0;
                $(inst.options.renderer.monthSelector, picker).each(
                    function () {
                        var nth = ++count % monthsToShow[1];
                        $(this).addClass(
                            nth === 1 ? "first" : nth === 0 ? "last" : "",
                        );
                    },
                );
            }
            ...
            picker
                ...
                .click(function () {
                    if (
                        !$(this).hasClass(inst.options.renderer.disabledClass)
                    ) {
                ...
                for (var day = 0; day < 7; day++) {
                    ...
                    days += this._prepare(renderer.day, inst).replace(
                        ...
                        (selectable
                            ...
                            '"' +
                            (dateInfo.title ||
                            (inst.options.dayStatus && selectable)
                                ? ' title="' +
                                  (dateInfo.title ||
                                      plugin.formatDate(
        ...
        _generateMonthSelection: function (
            ...
            for (var m = 1; m <= 12; m++) {
                if (
                    (!minDate ||
                        plugin
                            .newDate(year, m, plugin.daysInMonth(year, m))
                            .getTime() >= minDate.getTime()) &&
                    (!maxDate ||
                        plugin.newDate(year, m, 1).getTime() <=
                            maxDate.getTime())
                ) {
                    selector +=
                        '<option value="' +
            ...
            if (yearRange === "any") {
                ...
                if (start.getTime() < end.getTime()) {
                    start = (
                        ...
                        maxDate && maxDate.getTime() < end.getTime()
                            ? maxDate
                            : end
                    ).getFullYear();
                    ...
                    start = (
                        maxDate && maxDate.getTime() < start.getTime()
                            ? maxDate
                            : start
                    ).getFullYear();
                    end = (
                        minDate && minDate.getTime() > end.getTime()
                            ? minDate
                            : end
                    ).getFullYear();
